...
function addClass(req, res, next) {
  ...
  req.getConnection(function(err, conn) {
    if (err) return next(err);
    let sql = "INSERT INTO classinfo(`name`,content,days,pnames,storeid) VALUES (?,?,?,?,?);";

    conn.query(sql, [name, content, days, pnames, storeid], function(err, rows) {
      if (err) {
        console.error("query error", err);
        res.send({
          code: 1,
...
function setClass(req, res, next) {
  ...
  req.getConnection(function(err, conn) {
    if (err) return next(err);
    let sql = "UPDATE classinfo SET `name` = ?, content = ?, days = ?, pnames = ?, storeid = ? WHERE id = ?;";
